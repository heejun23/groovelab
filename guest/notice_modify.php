...
<body>
  ...
  <main>
    <section class="qna_upload">
      <h2 class="blind">공지사항수정</h2>
      <!-- notice 질문 제목 -->
      <form action="./php/notice_modify_check.php?no_no=<?php echo $row['no_no'] ?>" name="qna_upload" method="post" id="qna_upload" onsubmit="return formCheck();" class="needs-validation" novalidate>
        <input type="hidden" name="cl_no" value="<?= $row['cl_no'] ?>">
        <div class="qna_upload_title_box">
          <label for="no_title">제목</label>
          <input type="text" name="no_title" id="no_title" placeholder="제목을 입력하세요." value="<?php echo $row['no_title'] ?>" required>
        ...
        <div class="qna_upload_txt_box">
          <textarea name="no_content" id="no_content" cols="30" rows="10" placeholder="질문의 내용을 작성해 주세요!" required><?php echo $row['no_content'] ?></textarea>
        ...
        <div class="qna_upload_btn_box">
          ...
          <input type="submit" name="qna_upload_btn" id="qna_upload_btn" value="수정완료">
        </div>
      </form>
    </section>
  </main>
