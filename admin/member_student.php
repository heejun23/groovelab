...
$query = "SELECT mb_no FROM gl_member where mb_level=1";
$query .= $add_query;
...
$query = "select * from gl_member where mb_no<='$start_no' and mb_level=1";
$query .= $add_query;
$query .= " order by mb_no desc limit $page_size";
$result = mysqli_query($conn, $query);
// 검색 입력까지 고려하여 한 페이지 게시글 갯수만큼 mb_no를 기준으로 내림차순한 글 목록

$sql = "select count(*) from gl_member where mb_level=1 and $field like '%" . $search_word . "%'";
...
?>
    ...
    <ul class="gnb">
      <li>
        ...
        <ul class="sub_menu">
          ...
          <li><a href="member_student.php" title="학생회원" class="now_link">- 학생회원</a></li>
        </ul>
      </li>
      <li>
  ...
  <p class="branch">회원관리 > 학생회원</p>
  <main>
    <section>
      <h2>학생회원</h2>
      <div class="member_list_wrap">
        <!-- 검색폼 -->
        <form action="./member_student.php" name="search" method="get" class="member_search">
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_array($result)) {
              ...
              <tr onclick="location.href='./member_list_view_s.php?mb_no=<?= $row['mb_no'] ?>'" class="member_tr_link">
                <td>
                  <div class="member_profile_photo"><img src="../guest/images/profile/<?= $row['mb_photo'] ?>" alt="프로필사진">
                  </div>
                  <?= $row['mb_name'] ?>
                </td>
                <td>
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"member_student.php?no=$prev_list&field=$field&search_word=$search_word\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"member_student.php?no=$page&field=$field&search_word=$search_word\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"member_student.php?no=$next_list&field=$field&search_word=$search_word\"><img src='./images/next_admin.svg'></a>";
          }
