    ...
    <div id="mypage" class="wrap">
      ...
      <section id="left">
        ...
        <div class="wrap2">
          ...
          <article id="sell_list">
            <?php
            $sql = "SELECT count(*) FROM gl_class_member WHERE cm_datetime BETWEEN DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW();";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_array($result);
            ...
            <?php
            $sql = "SELECT * FROM gl_class_member WHERE cm_datetime BETWEEN DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW() ORDER BY cm_no DESC limit 2;";
            $result = mysqli_query($conn, $sql);
            $rowCount = 0;
          ...
          <article id="goal">
            <h2>올해 목표 매출액</h2>
            <a href="#" title="올해 목표 매출액"><img src="./images/plus_white.svg" alt="더보기버튼" class="plus"></a>
            <div class="text">
              <p><span class="green"></span>클래스</p>
              <p><span class="red"></span>스토어</p>
            </div>
            <div class="chart">
        ...
        <article id="new_class">
          <h2>새로 등록된 클래스</h2>
          <a href="class_ad_ongoing.php" title="새로 등록된 클래스"><img src="./images/plus_white.svg" alt="더보기버튼"
              class="plus"></a>
          <div class="flex_box">
            <?php
            $sql = "SELECT * from gl_class where cl_start<=curdate() order by cl_start desc limit 3;";
            $result = mysqli_query($conn, $sql);
            $rowCount = 0;
            while ($row = mysqli_fetch_array($result)) {
              $rowCount++;
      ...
      <section id="right">
        ...
        <article id="notice">
          ...
          <div class="wrap2">
            <?php
            $sql = "SELECT * FROM gl_notice order by no_no desc limit 5";
            $result = mysqli_query($conn, $sql);
            $rowCount = 0;
            while ($row = mysqli_fetch_array($result)) {
              $rowCount++;
        ...
        <article id="qna">
          ...
          <table>
            <?php
            $sql = "SELECT * from gl_qna where qna_parent_no is null order by qna_no desc limit 5";
            $result = mysqli_query($conn, $sql);
            $rowCount = 0;
            while ($row = mysqli_fetch_array($result)) {
              $rowCount++;
        ...
        <article id="new_review">
          ...
          <table>
            <?php
            $sql = "SELECT * from gl_review order by review_no desc limit 3;";
            $result = mysqli_query($conn, $sql);
            $rowCount = 0;
            while ($row = mysqli_fetch_array($result)) {
              $rowCount++;
