...
$query = "SELECT mb_no FROM gl_member where mb_level=2";
$query .= $add_query;
...
$query = "select * from gl_member where mb_no<='$start_no' and mb_level=2";
$query .= $add_query;
$query .= " order by mb_no desc limit $page_size";
$result = mysqli_query($conn, $query);
// 검색 입력까지 고려하여 한 페이지 게시글 갯수만큼 mb_no를 기준으로 내림차순한 글 목록

$sql = "select count(*) from gl_member where mb_level=2 and $field like '%" . $search_word . "%'";
...
?>
    ...
    <ul class="gnb">
      <li>
        ...
        <ul class="sub_menu">
          ...
          <li><a href="member_creator.php" title="크리에이터" class="now_link">- 크리에이터</a></li>
  ...
  <p class="branch">회원관리 > 크리에이터</p>
  <main>
    <section>
      <h2>크리에이터</h2>
      <div class="member_list_wrap">
        <!-- 검색폼 -->
        <form action="./member_creator.php" name="search" method="get" class="member_search">
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"member_creator.php?no=$prev_list&field=$field&search_word=$search_word\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"member_creator.php?no=$page&field=$field&search_word=$search_word\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"member_creator.php?no=$next_list&field=$field&search_word=$search_word\"><img src='./images/next_admin.svg'></a>";
          }
