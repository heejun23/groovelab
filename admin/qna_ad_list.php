...
<head>
  ...
  <link rel="stylesheet" href="./css/qna_ad_list.css" type="text/css">
  <!-- 파비콘 -->
...
if ($mb_level == 2) {
  $query = "SELECT qna_no FROM gl_qna where cl_no in (select cl_no from gl_class where mb_no = '$mb_no') AND qna_parent_no IS NULL";
} else if ($mb_level == 3) {
  $query = "SELECT qna_no FROM gl_qna WHERE qna_parent_no IS NULL";
}
if ($cl_no != "") {
  $query .= " AND cl_no = $cl_no";
}
$query .= " ORDER BY qna_no DESC LIMIT $no,1";
...
$query = "SELECT * from gl_qna where qna_no<='$start_no' AND qna_parent_no IS NULL";
$query .= $add_query;
$query .= $add_query2;
...
if ($mb_level == 2) {
  $sql = "SELECT count(*) from gl_qna where cl_no in (select cl_no from gl_class where mb_no = '$mb_no') AND qna_parent_no IS NULL";
} else if ($mb_level == 3) {
  $sql = "SELECT count(*) from gl_qna WHERE qna_parent_no IS NULL";
}
if ($cl_no != "") {
  $sql .= " AND cl_no = '$cl_no'";
} // 검색어가 있을 경우 추가할 쿼리문 작성
...
?>
  ...
  <main>
    <section>
      ...
      <div class="qna_wrap">
        <!-- 검색폼 -->
        <form name="search" method="get" class="qna_select">
          ...
          <input type="submit" value="이동하기" formaction="./qna_ad_list.php" class="qna_search_btn">
        </form>
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_array($result)) {
              ...
              <tr>
                ...
                <td><?= substr($row['qna_datetime'], 0, 10) ?></td>
                <td>
                  <?php
                  // 답변이 있는지 확인하기
                  $qna_no = $row['qna_no'];
                  $sql = "select * from gl_qna where qna_parent_no = $qna_no;";
                  $answer_result = mysqli_query($conn, $sql);
                  $answer = mysqli_fetch_assoc($answer_result);

                  if ($answer) { //답변이 있으면
                    ?>
                    <a href="./qna_ad_upload.php?type=modify&qna_no=<?= $row['qna_no'] ?>" title="답글수정"
                      class="qna_modify_btn">답글수정</a>
                  <?php } else { ?>
                    <a href="./qna_ad_upload.php?type=update&qna_no=<?= $row['qna_no'] ?>" title="답글쓰기"
                      class="qna_update_btn">답글쓰기</a>
                  <?php } ?>
                  <a href="./php/qna_ad_delete.php?qna_no=<?= $row['qna_no'] ?>" onclick="return delete_check()"
                    class="qna_delete_btn">질문삭제</a>
                </td>
              </tr>
              <?php
            }
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"qna_ad_list.php?no=$prev_list&cl_no=$cl_no\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"qna_ad_list.php?no=$page&cl_no=$cl_no\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"qna_ad_list.php?no=$next_list&cl_no=$cl_no\"><img src='./images/next_admin.svg'></a>";
          }
