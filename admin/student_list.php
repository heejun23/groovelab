...
// LEFT JOIN gl_class c ON cm.cl_no = c.cl_no WHERE c.mb_no = '$mb_no' AND cm.cm_status != 3  GROUP BY m.mb_no ORDER BY cm.cm_no DESC LIMIT $no,1";
// $query = "SELECT cm.cm_no 
// FROM gl_member m 
// INNER JOIN gl_class_member cm ON m.mb_no = cm.mb_no 
// LEFT JOIN gl_class c ON cm.cl_no = c.cl_no 
// WHERE c.mb_no = '$mb_no' AND cm.cm_status != 3 AND mb_level = 1 
// AND (m.$field LIKE '%" . $search_word . "%') 
// GROUP BY m.mb_no 
...
$query .= $add_query;
$query .= " ORDER BY cm_no DESC limit $no, 1";
$result = mysqli_query($conn, $query);
$row = mysqli_fetch_row($result);
$start_no = $row[0] ?? "0";

$query = "WITH RankedMembers AS ( SELECT m.mb_photo, m.mb_name, m.mb_level, m.mb_nick, m.mb_tel, m.mb_email, m.reg_date, c.cl_no, cm.cm_no, ROW_NUMBER() OVER (PARTITION BY m.mb_no ORDER BY cm.cm_no DESC) AS rn FROM gl_member m INNER JOIN gl_class_member cm ON m.mb_no = cm.mb_no LEFT JOIN gl_class c ON cm.cl_no = c.cl_no WHERE c.mb_no = '$mb_no' AND cm.cm_status != 3 AND m.mb_level = 1 ) SELECT mb_photo, mb_name, mb_level, mb_nick, mb_tel, mb_email, reg_date, cl_no FROM RankedMembers WHERE rn = 1 AND cm_no <= '$start_no' ";
$query .= $add_query;
  ...
  <header>
    ...
    <ul class="gnb">
      <li>
        ...
        <ul class="sub_menu">
          <li><a href="student_list.php" title="전체수강생" class="now_link">- 전체수강생</a></li>
      ...
      <div class="member_list_wrap">
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"student_list.php?no=$prev_list&field=$field&search_word=$search_word\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"student_list.php?no=$page&field=$field&search_word=$search_word\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"student_list.php?no=$next_list&field=$field&search_word=$search_word\"><img src='./images/next_admin.svg'></a>";
          }
