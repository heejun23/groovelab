...
// 멤버 레벨별로 다르게 출력 쿼리 - 첫번째 게시글 찾는 쿼리
if ($mb_level == 2) {
  $query = "SELECT cl_no FROM gl_class WHERE cl_end < '$today' AND mb_no = $mb_no AND $field like '%" . $search_word . "%' ORDER BY cl_no DESC LIMIT $no,1";
} else if ($mb_level == 3) {
  $query = "SELECT cl_no FROM gl_class WHERE cl_end < '$today' AND $field like '%" . $search_word . "%' ORDER BY cl_no DESC LIMIT $no,1";
}
...
if ($mb_level == 2) {
  $query = "select * from gl_class WHERE cl_end < '$today' AND cl_no<='$start_no' AND mb_no = $mb_no";
} else if ($mb_level == 3) {
  $query = "select * from gl_class WHERE cl_end < '$today' AND cl_no<='$start_no'";
}
...
if ($mb_level == 2) {
  $sql = "SELECT count(*) from gl_class WHERE cl_end < '$today' AND mb_no = $mb_no and $field like '%" . $search_word . "%'";
} else if ($mb_level == 3) {
  $sql = "select count(*) from gl_class WHERE cl_end < '$today' and $field like '%" . $search_word . "%'";
}
...
?>
      ...
      <ul class="gnb">
        ...
        <li>
          ...
          <ul class="sub_menu">
            ...
            <li><a href="class_ad_closed.php" title="종료클래스" class="now_link">- 종료클래스</a></li>
          </ul>
        </li>
        <li>
  ...
  <p class="branch">홈 > 클래스관리 > 종료클래스</p>
  ...
  <main>
    <section>
      ...
      <div class="class_list_wrap">
        <!-- 검색폼 -->
        <form action="./class_ad_closed.php" name="search" method="get" class="class_search">
              ...
              <tr>
                <td class="class_profile_Box">
                  ...
                  <p class="class_ad_title">
                    <?= $row['cl_category'] ?><br>
                    <a href="./class_ad_view.php?cl_no=<?= $row['cl_no'] ?>"><span><?= $row['cl_title'] ?></a></span>
                  </p>
                </td>
                ...
                if ($mb_level == 3) {
                  ...
                  <?php } else if ($date1 > $row['cl_end']) {
                    $query = "SELECT * from gl_class_member where cl_no = $cl_no AND cm_status=1";
                    $end_result = mysqli_query($conn, $query);
                    $end_row = mysqli_fetch_row($end_result);
                    if ($end_row) {
                      ?>
                        <td class="class_ad_btn">
                          <a href="./php/class_ad_end.php?cl_no=<?= $row['cl_no'] ?>" id="class_ad_modi">클래스종료</a>
                        </td>
                    <?php } else { ?>
                        <td class="class_ad_btn">
                          <a href="#" disabled class="class_ad_end">클래스 종료 완료</a>
                        </td>
                    <?php }
                  } else { ?>
                      <td class="class_ad_btn">
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"class_ad_closed.php?no=$prev_list&field=$field&search_word=$search_word\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"class_ad_closed.php?no=$page&field=$field&search_word=$search_word\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"class_ad_closed.php?no=$next_list&field=$field&search_word=$search_word\"><img src='./images/next_admin.svg'></a>";
          }
