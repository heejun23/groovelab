...
if ($mb_level == 2) {
  $query = "SELECT cl_no FROM gl_class WHERE mb_no = $mb_no";
  if ($search_word != "") {
    $query .= " and $field like '%" . $search_word . "%'";
  }
  $query .= " ORDER BY cl_no DESC LIMIT $no,1";
} else if ($mb_level == 3) {
  $query = "SELECT cl_no FROM gl_class";
  if ($search_word != "") {
    $query .= " WHERE $field like '%" . $search_word . "%'";
  }
  $query .= " ORDER BY cl_no DESC LIMIT $no,1";
}
...
if ($mb_level == 2) {
  ...
  $query = "select * from gl_class where cl_no<='$start_no'";
}
$query .= $add_query;
...
if ($mb_level == 2) {
  $sql = "select count(*) from gl_class where $field like '%" . $search_word . "%' AND mb_no = $mb_no";
} else if ($mb_level == 3) {
  $sql = "select count(*) from gl_class where $field like '%" . $search_word . "%'";
}
...
?>
    ...
    <header>
      ...
      <ul class="gnb">
        ...
        <li>
          ...
          <ul class="sub_menu">
            <li><a href="class_ad_list.php" title="전체클래스" class="now_link">- 전체클래스</a></li>
  ...
  <p class="branch">홈 > 클래스관리 > 전체클래스</p>
  ...
  <main>
    <section>
      ...
      <div class="class_list_wrap">
        <!-- 검색폼 -->
        <form action="./class_ad_list.php" name="search" method="get" class="class_search">
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"class_ad_list.php?no=$prev_list&field=$field&search_word=$search_word\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"class_ad_list.php?no=$page&field=$field&search_word=$search_word\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"class_ad_list.php?no=$next_list&field=$field&search_word=$search_word\"><img src='./images/next_admin.svg'></a>";
          }
