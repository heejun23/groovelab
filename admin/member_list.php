...
$query = "SELECT mb_no FROM gl_member";
if ($search_word != "") {
  $query .= " WHERE $field like '%" . $search_word . "%'";
}
$query .= " ORDER BY mb_no DESC LIMIT $no,1";
...
$query = "select * from gl_member where mb_no<='$start_no'";
$query .= $add_query;
$query .= " order by mb_no desc limit $page_size";
$result = mysqli_query($conn, $query);
// 검색 입력까지 고려하여 한 페이지 게시글 갯수만큼 mb_no를 기준으로 내림차순한 글 목록

$sql = "select count(*) from gl_member where $field like '%" . $search_word . "%'";
...
?>
  ...
  <header>
    ...
    <ul class="gnb">
      <li>
        ...
        <ul class="sub_menu">
          <li><a href="member_list.php" title="전체회원" class="now_link">- 전체회원</a></li>
  ...
  <p class="branch">회원관리 > 전체회원</p>
  <main>
    <section>
      <h2>전체회원</h2>
      <div class="member_list_wrap">
        <!-- 검색폼 -->
        <form action="./member_list.php" name="search" method="get" class="member_search">
        ...
        <table class="member_list">
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_array($result)) {
              if ($row['mb_level'] == 1) {
                ?>
                <tr onclick="location.href='./member_list_view_s.php?mb_no=<?= $row['mb_no'] ?>'" class="member_tr_link">
                <?php } else if ($row['mb_level'] == 2) { ?>
                  <tr onclick="location.href='./member_list_view_t.php?mb_no=<?= $row['mb_no'] ?>'" class="member_tr_link">
                <?php } else { ?>
                  <tr>
                <?php } ?>
                <td>
                  <div class="member_profile_photo"><img src="../guest/images/profile/<?= $row['mb_photo'] ?>"
                      alt="프로필사진">
                  </div>
                  <?= $row['mb_name'] ?>
                </td>
                ...
                <td><?= $row['mb_nick'] ?></td>
                <td>
                  <?= preg_replace("/([0-9]{3})([0-9]{4})([0-9]{4})/", "$1-$2-$3", $row['mb_tel']); ?>
                </td>
                <td><?= $row['mb_email'] ?></td>
        ...
        <div class="pagination">
          ...
          if ($start_page >= $page_list_size) {
            ...
            echo "<a href=\"member_list.php?no=$prev_list&field=$field&search_word=$search_word\"><img src='./images/prev_admin.svg'></a>";
          }
          ...
          for ($i = $start_page; $i <= $end_page; $i++) {
            ...
            if ($no != $page) {
              echo "<a href=\"member_list.php?no=$page&field=$field&search_word=$search_word\">" . $page_num . "</a>";
            } else {
          ...
          if ($total_page > $end_page) {
            ...
            echo "<a href=\"member_list.php?no=$next_list&field=$field&search_word=$search_word\"><img src='./images/next_admin.svg'></a>";
          }
