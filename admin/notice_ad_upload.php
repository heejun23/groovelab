...
<head>
  ...
  <link rel="stylesheet" href="./css/notice_ad_upload.css" type="text/css">
  <!-- 파비콘 -->
...
if ($type == 'update') {
  ...
  $title = '공지사항 작성';
  $action = './php/notice_ad_update.php';
  $submit = '작성하기';
} else if ($type == 'modify') {
  $no_no = $_GET['no_no'];
  ...
  $title = '공지사항 수정';
  $action = './php/notice_ad_modify.php';
  $submit = '수정하기';
}
?>
  ...
  <p class="branch">게시판관리 > 공지사항 > <?= $title ?></p>
  <main>
    <section>
      <h2><?= $title ?></h2>
      <div class="notice_wrap">
        <form action="<?= $action ?>" name="notice" method="post">
          <p>
            ...
            <input type="text" name="no_title" id="no_title" placeholder="제목을 작성해 주세요."
              value="<?= $row['no_title'] ?? ""; ?>" required maxlength="50">
          </p>
          <p>
            <?php
            $no_w_no = $row['mb_no'] ?? $mb_no;
            ...
            <input type="hidden" name="mb_no" id="mb_no" value="<?= $no_w_no ?>">
            <input type="text" name="writer" id="writer" value="<?= $writer[0] ?>" readonly>
          </p>
          <p>
            <?php
            $no_cl_no = $row['cl_no'] ?? $cl_no;
          ...
          <?php if ($type == "modify") { ?>
            <p>
              <input type="hidden" name="no_no" value="<?= $no_no ?>">
              <label for="no_datetime">작성일</label>
              <input type="text" name="no_datetime" id="no_datetime" value="<?= substr($row['no_datetime'], 0, 10) ?>"
                readonly>
            </p>
          <?php } ?>
          <p>
            <label for="no_content">내용</label>
            <textarea name="no_content" id="no_content" cols="30" rows="10" placeholder="내용을 작성해 주세요"
              required><?= $row['no_content'] ?? "" ?></textarea>
          </p>
